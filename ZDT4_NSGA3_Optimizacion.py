...
for i in range (0,5):
    ...
    Archivo_Simulacion     = f"Simulacion_ZDT4_{Abreviatura[i]}.xlsx"
    Archivo_Semilla        = f"Semilla_ZDT4_{Abreviatura[i]}_VD.json"
    Nombre_Salida          = f"ZDT4_NSGA3_Salida_{Abreviatura[i]}.xlsx"
    Nombre_Soluciones      = f"ZDT4_NSGA3_Soluciones_{Abreviatura[i]}.xlsx"
    ...
    # El siguiente código Modela el Problema ZDT4 con restricción
    class MyProblemZDT4(ElementwiseProblem):

        def __init__(self):
            super().__init__(n_var=10,
                             n_obj=2,
                             n_ieq_constr=1,
                             xl=np.array([0,-10,-10,-10,-10,-10,-10,-10,-10,-10]),
                             xu=np.array([1,10,10,10,10,10,10,10,10,10]))
    ...
    Evaluator().eval(problemZDT4, Pop_Semilla)
...
Duraciones_Tabla.to_excel('ZDT4_NSGA3_Duraciones.xlsx',index=True)