...
for i in range (0,5):
    ...
    Archivo_Simulacion     = f"Simulacion_ZDT2_{Abreviatura[i]}.xlsx"
    Archivo_Semilla        = f"Semilla_ZDT2_{Abreviatura[i]}_VD.json"
    Nombre_Salida          = f"Salida_NSGAIII_ZDT2_{Abreviatura[i]}.xlsx"
    Nombre_Soluciones      = f"Soluciones_NSGAIII_ZDT2_{Abreviatura[i]}.xlsx"
    ...
    # El siguiente código Modela el Problema ZDT2 con restricción
    class MyProblemZDT2(ElementwiseProblem):
    ...
    Evaluator().eval(problemZDT2, Pop_Semilla)
...
Duraciones_Tabla.to_excel('DuracionesZDT2.xlsx',index=True)